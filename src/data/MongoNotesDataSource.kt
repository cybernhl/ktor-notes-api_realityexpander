...
class MongoNotesDataSource : NotesDataSource {
...
// MONGO_HOST_POSTFIX=/?authSource=admin                   // Note: stripped out the "authmechanism" param
    ...
    override  suspend fun saveNote(note: Note): Boolean {
        if (note.id == null || note.id.isBlank()) {
            return notes.insertOne(note)
                .wasAcknowledged() // inserting will automatically set the id of the new note
        }
        ...
        return if (noteExists) {
            ...
            notes.insertOne(note)
                .wasAcknowledged()  // if note id is supplied, but doesn't exist, insert it anyway
        }
    }
    ...
    override   suspend fun deleteNoteForUser(userId: String, noteId: String): Boolean {
        ...
        return notes.updateOneById(noteId, note.copy(owners = note.owners - userId))
            .wasAcknowledged()
    }
    ...
    override suspend fun addOwnerToNote(userId: String, noteId: String): Boolean {
